...
inline fun <S, E, I : Any> MviDelegate<S, E, I>.launchTask(
    context: CoroutineContext = EmptyCoroutineContext,
    crossinline block: suspend () -> Unit,
): Job = scope.viewModelScope.launch(context = context) { block() }

inline fun <S, E, I : Any> MviDelegate<S, E, I>.launchTaskIf(
    condition: Boolean,
    context: CoroutineContext = EmptyCoroutineContext,
    crossinline block: suspend () -> Unit,
): Job? = if (condition) launchTask(context, block) else null

inline fun <S, E, I : Any, T> MviDelegate<S, E, I>.asyncTask(
    context: CoroutineContext = EmptyCoroutineContext,
    crossinline block: suspend () -> T,
): Deferred<T> = scope.viewModelScope.async(context = context) { block() }

inline fun <S, E, I : Any, T> MviDelegate<S, E, I>.asyncTaskIf(
    condition: Boolean,
    context: CoroutineContext = EmptyCoroutineContext,
    crossinline block: suspend () -> T,
